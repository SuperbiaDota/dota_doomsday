...
require('libraries/animations/animations')
-- This library can be used for performing "Frankenstein" attachments on units
--require('libraries/attachments')
-- This library can be used to synchronize client-server data via player/client-specific nettables
require('libraries/playertables')
-- This library can be used to create container inventories or container shops
require('libraries/containers/containers')
...
require( "scripts/vscripts/libraries/filters" )
require( "scripts/vscripts/libraries/talent/talent" )

-- adds crafting above the inventory ui
-- define the material droprates in npc_items_mat_drop.txt and the crafting recipes in npc_craft_recipes.txt
require( "libraries/craft/craft" )

require( "scripts/vscripts/util/util" )
...
function GameMode:OnHeroInGame(hero)
    ...
    hero:AddAbility("custom_ability_burst")
    hero:AddAbility("custom_ability_craft")

    hero:AddNewModifier(
          hero, -- player source
          nil, -- ability source
          "custom_modifier_hero_innate", -- modifier name
          {} -- kv
      )

    hero.BB_Multiplier = 2
    hero.max_streak = 0
end
...
function GameMode:OnGameInProgress()
    ...
    LaneCreepLogic:SendWaves(1)
    Timers:CreateTimer(28, -- Start this timer 28 game-time seconds later
        function()
            LaneCreepLogic:SpawnWaves()
...
function GameMode:InitGameMode()
    ...
    for _, building in pairs(all_buildings) do
        if building:IsTower() then
            local regen_mod = building:FindModifierByName("modifier_tower_aura")
            if regen_mod then regen_mod:Destroy() end
        end
    end
