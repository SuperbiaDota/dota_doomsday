...
function Projectiles:CreateTimer(handle, args)
    if not args.endTime or not args.callback then
        print("Invalid timer created: ".. handle)
        return
    end

    Projectiles.timers[handle] = args
end

function Projectiles:RemoveTimer(handle)
    Projectiles.timers[handle] = nil
end
...
function Projectiles:CreateProjectile(projectile)
    ...
    projectile.MovementType = projectile.MovementType or PROJECTILES_LINEAR
    ...
    if projectile.MovementType == PROJECTILES_ARC then
        projectile.travel_time = (projectile.fDistance / #projectile.vVelocity)
        projectile.destination = projectile.vSpawnOrigin + projectile.vVelocity * projectile.travel_time
        projectile.initial_z = ((projectile.fDestHeight - projectile.vSpawnOrigin.z) / projectile.travel_time + (projectile.fGravity * projectile.travel_time/2))
    end
    ...
    function projectile:Destroy()
        ParticleManager:DestroyParticle(projectile.id, projectile.bDestroyImmediate)
        Projectiles:RemoveTimer(projectile)
    end

    --projectile.ProjectileTimerName = DoUniqueString('proj')
    Projectiles:CreateTimer(projectile, {
        endTime = GameRules:GetGameTime(),
        useGameTime = true,
        callback = function()
            local curTime = GameRules:GetGameTime()

            if projectile.MovementType ~= PROJECTILES_LINEAR then
                if projectile.MovementType == PROJECTILES_ARC then
                    local elapsed = curTime - projectile.spawnTime
                    projectile.vel.z = (projectile.initial_z - projectile.fGravity * elapsed) / 30
                end
            end
            ...
            if projectile.Think then 
                local status, out = pcall(projectile.Think, projectile)
                if not status then
                    print('[PROJECTILES] Projectile Think Failure!: ' .. out)
                    projectile:Destroy()
                end
            end
    ...
    self.projectile_table[projectile] = true
    return projectile
end
