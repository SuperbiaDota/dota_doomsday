...
function TalentSystem:Init()
	...
	-- get Talent KVs
	local abilities = LoadKeyValues( "scripts/npc/npc_talents_custom.txt" )
	self.kv = {}
...
function TalentSystem:OnAbilityLearned( params )
	...
	-- update pentagram (looks for the number in the ability name)
	local talent_index = params.abilityname:sub(-1) - 1
	-- print("sending event for talent " .. talent_index)
	CustomGameEventManager:Send_ServerToPlayer(player, "talent_learned", {talent_index = talent_index})
end
...
function TalentSystem:UnitHasTalent( unit, name )
	...
	for _,mod in pairs(mods) do
		if mod:GetAbility() ~= nil and mod:GetAbility():GetAbilityName()==name then
			return true
		end
	end
	return false
